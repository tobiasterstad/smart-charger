...
def set_now(monkeypatch, fixed_dt: datetime.datetime):
...
def _count_hours_in_steps(steps: List[planner.ChargingStep], current: int) -> float:
    return sum((s.stop_time - s.start_time).total_seconds() / 3600 for s in steps if s.current == current)


def test_price_aware_picks_night_hours_when_cheaper(monkeypatch):
    # now = 2026-01-22 18:30
    fixed_dt = datetime.datetime(2026, 1, 22, 18, 30)
    set_now(monkeypatch, fixed_dt)
...
def test_price_aware_groups_adjacent_hours(monkeypatch):
...
def test_price_aware_fallback_no_provider(monkeypatch):
...
def test_price_aware_not_enough_hours(monkeypatch):
    ...
    now = planner.datetime.datetime.now()
    if now.hour < 7:
