...
class TestOperatingModeFromName(unittest.TestCase):
    def test_exact_name(self):
        self.assertEqual(
            OperatingMode.from_name("Connected_Charging"),
            OperatingMode.Connected_Charging,
        )

    def test_lowercase_with_spaces(self):
        self.assertEqual(
            OperatingMode.from_name("connected charging"),
            OperatingMode.Connected_Charging,
        )
    ...
    def test_substring_matches(self):
        ...
        self.assertEqual(
            OperatingMode.from_name("charging"), OperatingMode.Connected_Charging
        )
        self.assertEqual(
            OperatingMode.from_name("requesting"), OperatingMode.Connected_Requesting
        )
    ...
    def test_unknown_string(self):
        self.assertEqual(
            OperatingMode.from_name("unrelated_mode"), OperatingMode.Unknown
        )
